...
export async function validateSessionToken(
  ...
  // If session token is expired, return null
  if (Date.now() >= session.expiresAt.getTime()) {
    await prisma.session.delete({ where: { id: sessionId } });
    return { session: null, user: null };
  }
  // If less than 15 days, refresh token
...
// Get current session before streamed down to browser
...
export const registerUser = async (credentials: SignUpValues) => {
  ...
  try {
    ...
    // Optional: Inspect error.code for better messages
    if (
      typeof error === "object" &&
      error !== null &&
      "code" in error &&
...
export const loginUser = async (credentials: LoginValues) => {
  ...
  // Generate, create and set session token in cookie
